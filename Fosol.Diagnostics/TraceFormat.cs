...
namespace Fosol.Diagnostics
{
        ...
        : IDisposable
    {
        ...
        private readonly List<Keywords.TraceKeyword> _Keywords = new List<Keywords.TraceKeyword>();
        ...
        private static IEnumerable<Keywords.TraceKeyword> Compile(string format)
        {
        ...
        /// <param name="format">Formatting string value.</param>
        /// <returns>Collection of Keywords.</returns>
        private static IEnumerable<Keywords.TraceKeyword> Compile(Common.Parsers.SimpleParser parser, string format)
        {
            ...
            foreach (var phrase in phrases)
            {
                ...
                Keywords.TraceKeyword keyword = null;
                ...
                else
                {
                    ...
                    if (type != null)
                    {
                        ...
                        else
                        {
                            // Use the correct constructor.
                            if (is_empty_constructor)
                                keyword = (Keywords.TraceKeyword)Activator.CreateInstance(type);
                            else if (type.HasTypeConstructor(typeof(string), typeof(System.Collections.Specialized.StringDictionary)))
                                keyword = (Keywords.TraceKeyword)Activator.CreateInstance(type, phrase.Text, key.Params);
                            else if (type.HasTypeConstructor(typeof(string)))
                                keyword = (Keywords.TraceKeyword)Activator.CreateInstance(type, phrase.Text);
                            else if (type.HasTypeConstructor(typeof(System.Collections.Specialized.StringDictionary)))
                                keyword = (Keywords.TraceKeyword)Activator.CreateInstance(type, key.Params);
                        }
                    }
                    else
                        throw new System.Configuration.ConfigurationErrorsException(string.Format(Resources.Strings.Configuration_Exception_Keyword_Does_Not_Exist, key.Name));
                }
